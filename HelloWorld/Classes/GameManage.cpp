...
void GameManage::genItems(){//随机生成数组
	do{
		for(int i=0;i<ITEMSROW;i++) 
			for(int j=0;j<ITEMSCOL;j++) //CCRANDOM_0_1 左闭右开
	...
	//遍历整个GameBoard
	 for(int i=0;i<ITEMSROW;i++)
		 for(int j=0;j<ITEMSCOL;j++){
			 ...
			 //bug#1 应该先判断边界，否则后判断边界,isSwap已交换但为进入if语句，不能交换回去
			 if(tmpNItem.colNo<ITEMSCOL && isSwapItem(tmpCItem,tmpNItem,false) )//检测交换是否成功
			...
			tmpNItem.rowNo=i+1;tmpNItem.colNo=j;//纵向交换
			 ...
			 if( tmpNItem.rowNo<ITEMSROW  && isSwapItem(tmpCItem,tmpNItem,false))//检测交换是否成功
					{swap(Items[tmpCItem.rowNo][tmpCItem.colNo],Items[tmpNItem.rowNo][tmpNItem.colNo]);//交换回去
